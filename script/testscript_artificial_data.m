...
for seednum = 4
% for seednum = maxseed:-1:1
    
    oldRng = rng();
    rng(seednum);
    display(sprintf('seednum %d', seednum));
...
%     display(sprintf('seednum %d out of %d', seednum, maxseed));
    ...
    for kidx = 1:length(kmat)
        ...
        save(strcat('fixingalpha_dataflag ', num2str(data_flag), 'seednum ', num2str(seednum), 'k ', num2str(k), '.mat'), 'results', 'Yraw', 'col', 'truex');
